  ...
  return (
    <div className="w-full max-w-[1200px] mx-auto py-8 px-4 flex flex-col gap-8 animate-fade-up">
      ...
      <div className="text-center max-w-2xl mx-auto space-y-2">
        <div className="inline-flex items-center gap-2 px-3 py-1 bg-[var(--color-card-snow)]/5 border border-[var(--color-cloud)]/20 text-[#D93B2D] text-xs font-mono font-bold uppercase tracking-widest">
          <Sparkles className="w-3.5 h-3.5 text-[#D93B2D]" />
          <span>PREDICTIVE ROUTE OPTIMIZER</span>
        </div>
        <h2 className="text-4xl sm:text-5xl font-serif font-black text-[var(--color-ink-black)] tracking-tight">
          Bypass Gridlock Before Maps Turn Red
        </h2>
        <p className="text-base font-serif text-[var(--color-steel-gray)]">
      ...
      <div className="bg-[var(--color-card-snow)] border border-[var(--color-cloud)]/15 p-6 md:p-8 shadow-sm flex flex-col gap-6">
          ...
          <div className="space-y-1.5">
            <div className="flex justify-between items-center">
              ...
              <button
                ...
                disabled={locating}
                className="text-[10px] font-mono font-bold text-[var(--color-ink-black)] hover:text-[#D93B2D] bg-[var(--color-paper-white)] hover:bg-[var(--color-ink-black)]/10 px-2 py-0.5 border border-[var(--color-cloud)]/20 cursor-pointer uppercase flex items-center gap-1 transition-colors"
              >
            ...
            <input
              ...
              className="w-full bg-[var(--color-paper-white)] border border-[var(--color-cloud)]/20 focus:border-[#D93B2D] px-4 py-3 text-sm font-sans text-[var(--color-ink-black)] outline-none transition-colors"
        ...
        <div className="flex flex-wrap items-center gap-2 pt-2 border-t border-[var(--color-cloud)]/10">
          <span className="text-xs text-[var(--color-ink-black)]/60 font-mono font-bold uppercase mr-2">Popular Presets:</span>
          {presetPairs.map((pair, idx) => (
            <button
              ...
              className="bg-[var(--color-paper-white)] hover:bg-[var(--color-ink-black)] text-[var(--color-ink-black)] hover:text-white border border-[var(--color-cloud)]/20 px-3 py-1.5 text-xs font-mono font-bold transition-colors cursor-pointer border-none"
            >
              {pair.from} → {pair.to}
            </button>
          ))}
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4 pt-4 border-t border-[var(--color-cloud)]/10">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="text-xs font-bold font-mono text-[var(--color-ink-black)] uppercase">Forecast Horizon:</div>
          ...
          <button
            ...
            disabled={loading}
            className="w-full sm:w-auto bg-[var(--color-ink-black)] hover:bg-[#D93B2D] text-white px-8 py-3 font-mono font-bold text-xs uppercase tracking-widest flex items-center justify-center gap-2 cursor-pointer shadow-sm transition-colors border-none"
          >
